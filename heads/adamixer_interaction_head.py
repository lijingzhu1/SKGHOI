...
class InteractionHead(Module):
    ...
    def preprocess(self,
        ...
        for b_idx, detection in enumerate(detections):
            ...
            if len(active_idx) == 0:
                ...
                active_idx = torch. argmax(scores.flatten()).item()
                # results.append(dict(
                #     boxes=torch.zeros(0, 4, device=device),
                #     labels=torch.zeros(0, self.num_cls, device=device),
                #     scores=torch.zeros(0, 0, device=device)))
            # else:
            results.append(dict(
    ...
    def forward(self,
        ...
        # logger.info(f'box_feature:{[(key,value.size()) for key,value in features.items()]}')
        # logger.info(f'box_labels nan values:{[box_pair_feature for box_pair_feature in box_labels]}')
        # logger.info(f'box_scores nan values:{[box_pair_feature for box_pair_feature in box_scores]}')
        # logger.info(f'box_coords nan values:{[box_pair_feature for box_pair_feature in box_coords]}')
        # logger.info(f'image_shapes nan values:{image_shapes}')
        box_features = self.box_roi_pool(features, box_coords, image_shapes)

        # for box_pair_feature in box_features:
        #     logger.info(f'produced the box_pair_features nan values: {box_pair_feature.size()}')
        ...
        # 
        # logger.info(f'box_scores nan values:{[torch.isnan(box_pair_feature).sum() for box_pair_feature in box_scores]}')
        # logger.info(f'box_pair_features:{[torch.isnan(box_pair_feature).sum() for box_pair_feature in box_pair_features]}')
        # for box_pair_feature in box_pair_features:
        #     if torch.isnan(box_pair_feature).sum().item()  > 0:
        #         logger.info(f'produced the box_pair_features nan values: {box_pair_feature}')
        #         box_pair_feature = torch.nan_to_num(box_pair_feature)
        #         box_pair_features.append(box_pair_feature)
        box_pair_features = torch.cat(box_pair_features)
        # logger.info(f'box_pair_features:{box_pair_features.size()}')
        # logger.info(f'box_pair_features:{torch.any(torch.isnan(box_pair_features))}')
        # logger.info(f'box_pair_features in forward:{torch.isnan(box_pair_features).sum().item()}')
    ...
    def forward(self,
        ...
        for b_idx, (coords, labels, scores, bbox_features) in enumerate(zip(box_coords, box_labels, box_scores,box_features)):
            ...
            for _ in range(self.num_iter):
                ...
                # if torch.isnan(messages_to_o).sum() > 0:
                # logger.info(f'messages_to_o nan values: {torch.isnan(messages_to_o).sum()}')

                node_encodings = self.norm_o(
                    node_encodings + messages_to_o
                )
                # if torch.isnan(node_encodings).sum() > 0:
                # logger.info(f'node_encodings nan values: {torch.isnan(node_encodings).sum()}')
            if targets is not None:
                all_labels.append(self.associate_with_ground_truth(
                    coords[x_keep], coords[y_keep], targets[b_idx])
                )
                
            box_att1 = self.attention_head(torch.cat([h_node_encodings[x_keep],node_encodings[y_keep]], 1),
                    box_pair_spatial_reshaped[x_keep, y_keep])
            # logger.info(f'box_att1 nan values: {torch.isnan(box_att1).sum()}')
            box_att2 = self.attention_head_g(global_features[b_idx, None],box_pair_spatial_reshaped[x_keep, y_keep])
            # logger.info(f'box_att2 nan values: {torch.isnan(box_att2).sum()}')
            box_all_att = torch.cat([box_att1,box_att2], dim=1)
            # logger.info(f'box_att_all nan values: {torch.isnan(box_all_att).sum()}')
            # logger.info(f'box_att_all size: {box_all_att.size()}')
            all_box_pair_features.append(box_all_att)
