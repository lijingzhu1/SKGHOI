...
class LinearhHead(Module):
    ...
    def forward(self,
        ...
        linearh= LinearH(ent_tot = 10, rel_tot = 10,trans_dim = self.trans_dim,out_channels = self.out_channels,\
                        roi_pool_size = self.roi_pool_size,node_encoding_size = self.node_encoding_size,\
                        p_norm = self.transh_p_norm,norm_flag = self.transh_norm_flag).to(self.device)
        all_head = torch.cat((head,tail),dim = 0)
        all_tail = torch.cat((tail,head),dim = 0)
        all_relation = torch.cat((relation,relation),dim = 0)
        head_entity,relation,relation_norm,tail_entity,transh_scores = linearh(all_head,all_relation,all_tail)
...
class GraphHead(Module):
    ...
    def __init__(self,
        ...
        # self.ConvH_head = ConvhHead(
        #     transh_dim = 1024,
        #     out_channels = self.out_channels,
        #     roi_pool_size = self.roi_pool_size,
        #     node_encoding_size = self.node_encoding_size,
        #     transh_p_norm = 2,
    ...
    def compute_prior_scores(self,
        ...
        # logger.info(f'self.object_class_to_target_class:{len(self.object_class_to_target_class)}')
    ...
    def forward(self,
        ...
        for b_idx, (coords, labels, scores, bbox_features) in enumerate(zip(box_coords, box_labels, box_scores,box_features)):
            ...
            transh_head_entitiy,transh_relations,transh_relations_norm,transh_tail_entity,transh_scores = self.trans_head(
                                                        x,y,n_h,n,x_keep,y_keep,\
                                                        h_node_encodings[x],box_pair_spatial,node_encodings[y],labels
                                                        )
            # p_t= int(transh_head_entitiy.shape(0))/2
            positive_graph_human_node,negative_graph_human_node = torch.split(transh_head_entitiy,[n_h*n,n_h*n])
            positive_graph_object_node,negative_graph_object_node = torch.split(transh_tail_entity,[n_h*n,n_h*n])
            positive_relation,negative_relation = torch.split(transh_relations,[n_h*n,n_h*n])
            positive_relation_norm,negative_relation_norm = torch.split(transh_relations_norm,[n_h*n,n_h*n])
            positive_transh_score,negative_transh_score = torch.split(transh_scores,[n_h*n,n_h*n])
            # logger.info(f'positive_graph_human_node:{positive_graph_human_node.size()}')
            box_pair_spatial_reshaped = positive_relation.reshape(n_h, n, -1)

            adjacency_matrix = torch.ones(n_h, n, device=device)
            for _ in range(self.num_iter):
                # Compute weights of each edge
                app = torch.cat([positive_graph_human_node,positive_graph_object_node], 1)
                
                weights = self.attention_head(app,positive_relation)
                adjacency_matrix = self.adjacency(weights).reshape(n_h, n)
            
                # Update human nodes
                index_o = int((positive_graph_object_node.size()[0]/n_h))
                index_h = [i for i in range(0,positive_graph_human_node.size()[0]) if i % n == 0]
                # logger.info(f'indics_h :{index_h}')
                all_entity_encoding = positive_graph_object_node[0:index_o]
                h_entity_encoding = positive_graph_human_node[index_h]
            ...
            transh_positive_scores_keep= positive_transh_score.reshape(n_h,n)[x_keep,y_keep]
            transh_negative_scores_keep= negative_transh_score.reshape(n_h,n)[x_keep,y_keep]
            # logger.info(f'transh_negative_scores_keep:{transh_negative_scores_keep.size()}')
            transh_positive_head_entitys_keep= positive_graph_human_node.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_negative_head_entitys_keep= negative_graph_human_node.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_positive_tail_entitys_keep= positive_graph_object_node.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_negative_tail_entitys_keep= negative_graph_object_node.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_positive_relations_keep = positive_relation.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_negative_relations_keep = negative_relation.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_positive_relations_norm_keep = positive_relation_norm.reshape(n_h,n,-1)[x_keep,y_keep]
            transh_negative_relations_norm_keep = negative_relation_norm.reshape(n_h,n,-1)[x_keep,y_keep]
