...
from adamixer_interaction_head import InteractionHead, GraphHead
...
class GenericHOINetwork(nn.Module):
    ...
    def forward(self,
        ...
        # logger.info(f'features nan values:{[torch.isnan(box_pair_feature).sum() for box_pair_feature in features]}')
        ...
        # logger.info(f'box_feature:{[(key,value) for key,value in box_feature.items()]}')
