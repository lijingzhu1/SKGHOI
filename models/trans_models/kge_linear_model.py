...
sys.path.append('/users/PCS0256/lijing/spatially-conditioned-graphs/heads/trans_heads')
from kge_linear_head import InteractionHead, GraphHead, LinearhHead
...
class SpatiallyConditionedGraph(GenericHOINetwork):
    def __init__(self,
        ...
        # Backbone parameters
        mmdetection_config: str,
        mmdetection_checkpoint: str,
        # Pooler parameters
        output_size: int = 7,
        sampling_ratio: int = 2,
        # Box pair head parameters
        node_encoding_size: int = 512,
        ...
        max_human: int = 15,
        max_object: int = 15,
        # Translational head parameters
        # translational_head = str,
        translational_dim = str,
        translational_p_norm = int,
        translational_norm_flag = bool,
        clip_model = str,
        ...
        cfg = mmcv.Config.fromfile(mmdetection_config)
        # detector = build_detector(cfg.model, train_cfg = cfg.get('train_cfg'), test_cfg=cfg.get('test_cfg'))
        detector = build_detector(cfg['model'])

        if mmdetection_checkpoint is not None:
            mmdetection_checkpoint = load_checkpoint(detector, mmdetection_checkpoint, map_location='cpu')
        ...
        translational_head = LinearhHead(
            # entity_size = entity_size,
            # relation_size = relation_size,
            entity_encoding_size = entity_encoding_size,
            relation_encoding_size_1 = relation_encoding_size_1,
            relation_encoding_size_2 = relation_encoding_size_2,
            trans_dim = translational_dim,
            # out_channels = detector_neck.out_channels,
            # roi_pool_size = output_size,
            # node_encoding_size = node_encoding_size,
            transh_p_norm = translational_p_norm,
            transh_norm_flag = translational_norm_flag,
            human_idx = human_idx,
        ...
        box_pair_head = GraphHead(
            ...
            trans_head = translational_head,
            obj_text_label = obj_text_label,
            hoi_text_label = hoi_text_label,
            hoi_valid_pair = hoi_valid_pair,
            hoi_action_ongoing_label = hoi_action_ongoing_label,
            hoi_action_valid_object = hoi_action_valid_object,
            clip_model = clip_model,
            spatial_encoded_dim = spatial_encoded_dim
        )
